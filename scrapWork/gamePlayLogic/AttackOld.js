...
var turn = function(){
   yourDeck = deck[$(getYou()).get(0);
   theirDeck = deck[$(getEnemy()).get(0);

   yourCard = deckObj[yourDeck.firstChild($('.selected')).get(0).getAttribute('value')];
   theirCard = deckObj[theirDeck.firstChild($('.selected')).get(0).getAttribute('value')];
   ...
   function totalDamage(pass this in event listener:attacker.id, attacker.attackchoice defender.id){

      // var ours = yourType[0];
      // var theirs = theirType[0];
      var ours = defecnder[deck][NAME OF CHAR FOR RIGHT PLAYER]['ATTACKNUM'][1];;
      var theirs = defecnder[deck][NAME OF CHAR FOR RIGHT PLAYER]['def'][1];
      var dmg = rollAttack();
